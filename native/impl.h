...
namespace native
{
    namespace impl
    {
        ...
        error fs_exec_async(F uv_function, C callback, P&&... params)
        {
            ...
            int res = uv_function(uv_default_loop(), req, std::forward<P>(params)..., on_fs_after);
            if(res < 0)
            {
        ...
        error fs_exec_sync(F uv_function, P&&... params)
        {
            ...
            int res = uv_function(uv_default_loop(), &req, std::forward<P>(params)..., nullptr);
            if(res < 0)
            {
                uv_fs_req_cleanup(&req);
                return error(uv_last_error(uv_default_loop()));
        ...
        error fs_read_link(const std::string& path, std::function<void(error, const std::string&)> callback=nullptr)
        {
            return fs_exec(uv_fs_readlink, callback, path.c_str());
        }
