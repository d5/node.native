...
namespace native
{
    namespace detail
    {
        ...
        private:
            client_context(native::detail::tcp* server)
                : socket_(nullptr)
                , parser_()
...
#if 0
        template<typename ...E>
        class server_response_t : public StreamT<false, true, E...>
        {
        public:
        };

        typedef server_response_t<> ServerResponse;
#endif
    }
}
