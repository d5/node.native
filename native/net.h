...
namespace native
{
	namespace net
	{
		...
		public:
			...
			{
				native::base::callbacks::store(get()->data, native::base::uv_cid_connect, callback);
				return uv_tcp_connect(new uv_connect_t, get<uv_tcp_t>(), to_ip4_addr(ip, port), [](uv_connect_t* req, int status) {
					native::base::callbacks::invoke<callback_t>(req->handle->data, native::base::uv_cid_connect, status);
					delete req;
				}) == 0;
			}
			...
			{
				native::base::callbacks::store(get()->data, native::base::uv_cid_connect6, callback);
				return uv_tcp_connect6(new uv_connect_t, get<uv_tcp_t>(), to_ip6_addr(ip, port), [](uv_connect_t* req, int status) {
					native::base::callbacks::invoke<callback_t>(req->handle->data, native::base::uv_cid_connect6, status);
					delete req;
				}) == 0;
			}
